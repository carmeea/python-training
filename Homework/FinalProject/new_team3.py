...
file_path = "data.csv"
output_path = "/home/aura/work/python-training/Homework/FinalProject"
    ...
    try:
        with open(file_path, "r") as file:
            ...
            products = [
                ...
                if selected_subcategory.lower()
                == row[1].split("/")[4].replace("-", " ").lower()
            ]
            return products
    except FileNotFoundError:
        print(f"File {file_path} not found. Aborting")
...
def write_to_csv(products, selected_subcategory):
    try:
        ...
        if os.path.exists(f"/{output_path}/{file_name}_products.csv"):
            print(f"{file_name}_products.csv already exists. Please try again")
        else:
            with open(
                f"{output_path}/{file_name}_products.csv", "w", newline=""
            ) as file:
            ...
            print(
                f"Products matching subcategory '{selected_subcategory}' have been written to '{selected_subcategory}_products.csv'"
            )
    except Exception as err:
